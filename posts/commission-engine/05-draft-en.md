...
- "quincena" → "pay period": "quincena" es un término de nómina semi-mensual muy usado en español (LatAm/España) sin equivalente de una sola palabra en inglés que preserve exactamente el mismo matiz (un "fortnight" en inglés implica dos semanas corridas, no un periodo fijo ligado al día 15/fin de mes). Se optó por "pay period"/"first two pay periods of the month", descriptivo pero preciso, en vez de forzar "fortnight" y confundir a una audiencia mayoritariamente estadounidense de desarrolladores.
- Recurso narrativo de cocina (recipe/kitchen/chef/stove/menu/dish): se evaluó si la analogía funciona igual de bien en inglés y la respuesta es sí — no depende de referencias gastronómicas específicas del español, así que se mantuvo literal en cada aparición (receta→recipe, cocina→kitchen, fogones→stove, plato→dish, carta/menú→menu, chef→chef) sin necesidad de sustituir el recurso narrativo por otro.
- "23:59 del día 20" → "11:59 PM on the 20th": conversión de formato de hora de 24h a 12h (AM/PM), convención estándar en inglés de EE.UU./Reino Unido para audiencias no técnicas de horarios; no altera el dato (mismo instante, mismo día del mes).
- "ceguera de legado" (Eric Evans) → "legacy blindness": no es una traducción nueva, sino la restitución del término original en inglés — Evans escribió su obra de referencia en inglés, así que se usó el término tal como aparece en la fuente en vez de retraducir desde el español.
- "trabajo codo a codo" → "working side by side": modismo traducido por su equivalente funcional en inglés (no literal "elbow to elbow", que sonaría forzado).
- "ecosistema tecnológico" → "technology stack": se adaptó a la terminología estándar que usaría la audiencia de desarrolladores/arquitectos en inglés; una traducción literal ("technological ecosystem") suena a relleno de marketing en inglés y podría rozar la regla de "sin jerga de marketing genérica" de la guía de estilo, mientras que "technology stack" es preciso y natural para ese público sin agregar ni quitar información.
- CTA de cierre: traducido de forma natural y no literal ("Si esto te suena familiar, así lo resolvimos nosotros: cuéntanos tu versión en los comentarios, o síguenos para más casos como este" → "If this sounds familiar, here's how we solved it — tell us your version in the comments, or follow us for more cases like this one"), preservando el mismo tono implícito del Patrón A y el mismo llamado a la acción doble (comentar / seguir en redes) de `marca.cta_tipica`, sin convertirlo en pregunta directa ni en banner explícito.
- No se usó `marca_en` porque `blogs.yaml` no define esa entrada para nitza-develop (comentario explícito en el YAML: "usar los mismos valores que marca"); se aplicaron los mismos `marca.voz` ("Técnica y cercana") y `marca.publico` (desarrolladores web y arquitectos de software) que en la versión en español, solo cambiando el idioma.
- Cifras y nombres propios sin alterar: £48.65 millones, £32 millones, TSB Bank, salida del CEO, ~$100 USD, "4 meses" de ciclos — se mantuvieron exactamente iguales, sin conversión de moneda ni redondeo, tal como exige la guía editorial para afirmaciones cuantitativas ya verificadas por el crítico.
-->
