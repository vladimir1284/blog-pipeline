...
# Killbill's Recipe vs. the Real Kitchen: Migrating Subscriptions Without Shutting Down the Restaurant
...
- Título: se pasó de "La receta de Killbill y la cocina real: migrar suscripciones sin apagar el restaurante" a "Killbill's Recipe vs. the Real Kitchen: Migrating Subscriptions Without Shutting Down the Restaurant" — traducción funcional, no literal, para que suene natural como titular en inglés sin perder la metáfora culinaria del recurso_narrativo.
- "la ventanilla de BigCommerce" -> "BigCommerce's front counter" en vez de "window/teller window": "ventanilla" evoca una ventanilla de banco/trámite en español, que no tiene la misma connotación en inglés; "front counter" mantiene la misma idea (pasar por el canal oficial, visible, en vez de una puerta trasera) y encaja mejor dentro del campo semántico de restaurante que ya trae el recurso_narrativo.
- "no llegaba con el paladar en blanco" -> "wasn't coming in with a blank palate": traducción casi directa; se conservó porque "paladar en blanco" (evocando "mente en blanco" en español) tiene un equivalente natural en inglés con "blank palate" dentro del mismo campo gastronómico, sin necesidad de buscar otra imagen.
- CTA final: "Cuéntanos en nuestras redes cómo resolviste tu propia versión de este problema — nos interesa comparar recetas" se tradujo como "Tell us on our social channels how you solved your own version of this problem — we'd love to compare recipes", una adaptación natural del registro (no literal de "nos interesa") que mantiene el mismo llamado a la acción y el mismo cierre culinario.
- No se identificaron modismos, anécdotas o referencias culturales del original que dependieran de un contexto específicamente hispanohablante (el caso del cliente de renta de trailers, BigCommerce, Braintree, Avalara y Killbill son entidades/hechos técnicos, no referencias culturales) — no hizo falta sustituir ningún hecho ni ejemplo, solo ajustar el idioma.
-->
