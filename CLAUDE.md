...
- **`blogs.yaml`**: per-blog brand data (`marca.voz`, `marca.recurso_narrativo` (optional recurring motif/analogy, e.g. culinary metaphors — kept separate from `voz` so it stays a distinct, reusable field per blog), `marca.publico`, `marca.evitar`, `marca.cta_tipica`, repo URL/branch/posts folder). Every agent that writes prose must look up its `blog_slug` here — brand tone and hard limits (`marca.evitar`) come from this file, not from the skills.
